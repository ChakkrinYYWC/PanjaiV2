...
server.post("/register", upload.single('IDcard'), function(req, res){
    ...
    user.register(new user({name: req.body.name, username: req.body.username, idcard: req.file.filename, email: req.body.email, address: req.body.address, phone: req.body.phone , accessToken: null , isbaned : "no"}), req.body.password,function(error, user){
        if(error){
